import sys
#from timo.read_gmx_ffbon_par import *
#from timo.read_gmx_ffnb_par import *
def s(string,length):
    string = str(string)
...
class topology:
    ...
    class atomtype:
        ...
        def cprint(self):
            return str(self.name) + s(self.bond_type,14) + s(self.mass,12) + s(self.charge,8) + s(self.ptype,3) + s(self.sigma,14) + s(self.epsilon,13)
...
def main(argv=sys.argv):
    if len(argv)<3: argv=['','name.gmx.top','name.gmx.ttt.top'];
    top = topology()
    top.read_top(argv[1])
    top.write_top(argv[2])
    sys.exit(0)
main()
'''